 ...
 * <p>
 * {@code StreamX3} enables index-aware, type-safe, and readable iteration over 3 lists in parallel.
 ...
 *
 * <pre>{@code
 * StreamX3<T1, T2, T3> stream =
 *     StreamX.addLists(list1, list2, list3);
 *
...
public class StreamX3<T1, T2, T3> {
    ...
    /**
     * Adds a new array of elements to this {@link StreamX3}, producing a {@link StreamX4}
     ...
     */
    public <T4> StreamX4<T1, T2, T3, T4> addList(List<T4> list) {
        if (list.size() != size) {
            throw new IllegalArgumentException("List size mismatch");
        }
        return new StreamX4<>(list1, list2, list3, list);
    }
     ...
     * @return a new {@code StreamX3<T1, T2, T3>} with filtered elements
     */
