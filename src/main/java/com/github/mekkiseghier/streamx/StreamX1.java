 ...
 * <p>
 * {@code StreamX1} enables index-aware, type-safe, and readable iteration over 1 lists in parallel.
...
public class StreamX1 < T1 > {
    ...
    /**
     * Adds a new array of elements to this {@link StreamX1}, producing a {@link StreamX2}
     ...
     * @return a {@code StreamX2<T1, T2>} representing the parallel lists
     * @throws IllegalArgumentException if the number of elements does not match the existing size
     */
    @SafeVarargs public final < T2 > StreamX2 < T1, T2 > addElements( T2... values ) {
        return new StreamX2 <>( list1, List.of( values ) );
    }
    /**
     * Adds a new list to this {@link StreamX1}, producing a {@link StreamX2} that enables
     ...
     */
    public < T2 > StreamX2 < T1, T2 > addList( List < T2 > list ) {
        return new StreamX2 <>( list1, list );
    }
    /**
     ...
     * @return a new {@code StreamX1<T1>} with filtered elements
     */
     ...
     */
    public StreamX1<T1> peek(Consumer1<Integer, T1> action) {
        for (int i = 0; i < list1.size(); i++) {
            action.accept(i, list1.get(i));
        }
        return this;
