...
import com.github.mekkiseghier.zipper.accumulators.Accumulator7;
import com.github.mekkiseghier.zipper.consumers.Consumer7;
import com.github.mekkiseghier.zipper.functions.Function7;
import com.github.mekkiseghier.zipper.predicates.Predicate7;
import com.github.mekkiseghier.zipper.tuples.Tuple7;

import java.util.ArrayList;
import java.util.List;

/**
 * A type-safe stream-like utility for working with 7 parallel lists of values.
 * <p>
 * {@code Zipper7} enables index-aware, type-safe, and readable iteration over 7 lists in parallel.
 ...
 *
 * <pre>{@code
 * Zipper7<T1, T2, T3, T4, T5, T6, T7> stream =
 *     Zipper.zipLists(list1, list2, list3, list4, list5, list6, list7);
 *
 * stream.forEach((i, v1, v2, v3, v4, v5, v6, v7) -> {
 *     System.out.println(i + ": " + v1 + ", " + v2 + ", " + v3 + ", " + v4 + ", " + v5 + ", " + v6 + ", " + v7");
 * });
 * }</pre>
...
public class Zipper7<T1, T2, T3, T4, T5, T6, T7> {
    ...
    public Zipper7(List<T1> list1, List<T2> list2, List<T3> list3, List<T4> list4, List<T5> list5, List<T6> list6, List<T7> list7) {
    ...
    /**
     * Adds a new array of elements to this {@link Zipper7}, producing a {@link Zipper8}
     * that enables parallel index-aware iteration over 8 sequences.
     *
     * <p>The number of provided elements must match the size of the existing lists to
     * maintain index alignment.</p>
     *
     * <pre>{@code
     * streamX7.zip(v1, v2, v3)
     *        .forEach((i, ...) -> { ... });
     * }</pre>
     *
     * @param values the array of new elements to include
     * @param <T8> the type of the new elements
     * @return a {@code Zipper8<T1, T2, T3, T4, T5, T6, T7, T8>} representing the parallel lists
     * @throws IllegalArgumentException if the number of elements does not match the existing size
     */
     @SafeVarargs    public final <T8> Zipper8 <T1, T2, T3, T4, T5, T6, T7, T8> zip8( T8... values) {
        ...
        return new Zipper8 <>(list1, list2, list3, list4, list5, list6, list7, safeList);
    }

    /**
     * Adds a new list to this {@link Zipper7}, producing a {@link Zipper8} that enables
     * parallel index-aware iteration over 8 lists.
     *
     * <p>All lists must have the same size to maintain index alignment.</p>
     *
     * <pre>{@code
     * streamX7.zipList(list8)
     *        .forEach((i, ...) -> { ... });
     * }</pre>
     *
     * @param list the new list to include in the stream
     * @param <T8> the type of elements in the new list
     * @return a {@code Zipper8<T1, T2, T3, T4, T5, T6, T7, T8>} representing the parallel lists
     * @throws IllegalArgumentException if the provided list size does not match existing lists
     */
    public <T8> Zipper8<T1, T2, T3, T4, T5, T6, T7, T8> zipList8(List<T8> list) {
        if (list.size() != size) {
            throw new IllegalArgumentException("List size mismatch");
        }
        return new Zipper8<>(list1, list2, list3, list4, list5, list6, list7, list);
    }

    /**
     * Performs the given action on each set of elements in parallel across 7 lists.
     * The index and corresponding elements are passed to the provided action.
     *
     * <pre>{@code
     * streamX7.forEach((i, a1, ..., a7) -> {
     *     // use i, a1, ..., a7
     * });
     * }</pre>
     *
     * @param action a lambda that receives the index and the 7 elements at that index
     */
    public void forEach7(Consumer7<Integer, T1, T2, T3, T4, T5, T6, T7> action) {
    ...
    /**
     * Maps each indexed group of 7 elements into a new result using the provided mapper.
     *
     * <pre>{@code
     * List<R> results = streamX7.map((i, a1, ..., a7) -> {
     *     return ...; // return type R
     * });
     * }</pre>
     *
     * @param mapper a function that receives the index and 7 elements, and returns a result
     * @param <R> the result type of the mapping function
     * @return a list of mapped results
     */
    public <R> List<R> map7(Function7<Integer, T1, T2, T3, T4, T5, T6, T7, R> mapper) {
     ...
     *
     * <pre>{@code
     * streamX7.filter((i, a1, ..., a7) -> condition)
     * }</pre>
     *
     * @param predicate the predicate to test each element group
     * @return a new {@code Zipper7<T1, T2, T3, T4, T5, T6, T7>} with filtered elements
     */
    public Zipper7<T1, T2, T3, T4, T5, T6, T7> filter7(Predicate7<Integer, T1, T2, T3, T4, T5, T6, T7> predicate) {
        ...
        return new Zipper7<>(filtered1, filtered2, filtered3, filtered4, filtered5, filtered6, filtered7);
    }
    /**
     * Reduces all elements into a single result using the given accumulator function.
     *
     * <pre>{@code
     * R result = streamX7.reduce(initialValue, (acc, i, a1, ..., a7) -> ...);
     * }</pre>
     *
     ...
     */
    public <R> R reduce7(R identity, Accumulator7<Integer, T1, T2, T3, T4, T5, T6, T7, R> accumulator) {
        R result = identity;
        for (int i = 0; i < size; i++) {
    ...
    /**
     * Converts this stream into a list of {@code Tuple7} objects,
     ...
     * </p>
     *
     * @return a list of {@code Tuple7<Integer, T1, T2, T3, T4, T5, T6, T7>} entries.
     */
    public List<Tuple7<T1, T2, T3, T4, T5, T6, T7>> asTupleList7() {
     ...
     */
    public boolean allMatch7(Predicate7<Integer, T1, T2, T3, T4, T5, T6, T7> predicate) {
     ...
     */
    public boolean anyMatch7(Predicate7<Integer, T1, T2, T3, T4, T5, T6, T7> predicate) {
        for (int i = 0; i < list1.size(); i++) {
     ...
     */
    public Zipper7<T1, T2, T3, T4, T5, T6, T7> peek7(Consumer7<Integer, T1, T2, T3, T4, T5, T6, T7> action) {
        for (int i = 0; i < list1.size(); i++) {
            action.accept(i, list1.get(i), list2.get(i), list3.get(i), list4.get(i), list5.get(i), list6.get(i), list7.get(i));
        }
        return this;
