...
;
     ...
     */
    @SafeVarargs public final < T3 > Zipper3 < T1, T2, T3 > addElements( T3... values ) {
     ...
     */
    public <T3> Zipper3<T1, T2, T3> addList(List<T3> list) {
        if (list.size() != size) {
            throw new IllegalArgumentException("List size mismatch");
        }
     ...
     */
    public Zipper2 < T1, T2 > filter( Predicate2 < Integer, T1, T2 > predicate ) {
     ...
     */
    public Zipper2<T1, T2> peek(Consumer2<Integer, T1, T2> action) {
        for (int i = 0; i < list1.size(); i++) {
            action.accept(i, list1.get(i), list2.get(i));
        }
        return this;
