...
import com.github.mekkiseghier.zipper.accumulators.Accumulator2;
import com.github.mekkiseghier.zipper.consumers.Consumer2;
import com.github.mekkiseghier.zipper.functions.Function2;
import com.github.mekkiseghier.zipper.predicates.Predicate2;
import com.github.mekkiseghier.zipper.tuples.Tuple2;
 ...
 * <p>
 * {@code Zipper2} enables index-aware, type-safe, and readable iteration over 2 lists in parallel.
 ...
 *
 * <pre>{@code
 * Zipper2<T1, T2> stream =
 *     Zipper.addLists(list1, list2);
 *
...
public class Zipper2 < T1, T2 > {
    private final List < T1 > list1;
    private final List < T2 > list2;
    private final int size;


    public Zipper2( List < T1 > list1, List < T2 > list2 ) {
...
;
    ...
    /**
     * Adds a new array of elements to this {@link Zipper2}, producing a {@link Zipper3}
     ...
     */
    public <T3> Zipper3<T1, T2, T3> zipList3(List<T3> list) {
        if (list.size() != size) {
            throw new IllegalArgumentException("List size mismatch");
        }
        return new Zipper3<>(list1, list2, list);
    }
     ...
     */
    public void forEach2(Consumer2<Integer, T1, T2> action) {
     ...
     * @return a list of mapped results
     */
    public <R> List<R> map2(Function2<Integer, T1, T2, R> mapper) {
     ...
     * @return a new {@code Zipper2<T1, T2>} with filtered elements
     */
    public Zipper2 < T1, T2 > filter2( Predicate2 < Integer, T1, T2 > predicate ) {
        ...
        return new Zipper2 <>( filtered1, filtered2 );
    }
    /**
     ...
     */
    public < R > R reduce2( R identity, Accumulator2 < Integer, T1, T2, R > accumulator ) {
        R result = identity;
        for ( int i = 0; i < size; i++ ) {
     ...
     */
    public List<Tuple2<T1, T2>> asTupleList2() {
     ...
     */
    public boolean allMatch2(Predicate2<Integer, T1, T2> predicate) {
     ...
     */
    public boolean anyMatch2(Predicate2<Integer, T1, T2> predicate) {
        for (int i = 0; i < list1.size(); i++) {
     ...
     */
    public Zipper2<T1, T2> peek2(Consumer2<Integer, T1, T2> action) {
        for (int i = 0; i < list1.size(); i++) {
            action.accept(i, list1.get(i), list2.get(i));
        }
        return this;
