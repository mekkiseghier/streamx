...
import com.github.mekkiseghier.zipper.accumulators.Accumulator5;
import com.github.mekkiseghier.zipper.consumers.Consumer5;
import com.github.mekkiseghier.zipper.functions.Function5;
import com.github.mekkiseghier.zipper.predicates.Predicate5;
import com.github.mekkiseghier.zipper.tuples.Tuple5;

import java.util.ArrayList;
import java.util.List;

/**
 * A type-safe stream-like utility for working with 5 parallel lists of values.
 * <p>
 * {@code Zipper5} enables index-aware, type-safe, and readable iteration over 5 lists in parallel.
 ...
 *
 * <pre>{@code
 * Zipper5<T1, T2, T3, T4, T5> stream =
 *     Zipper.zipLists(list1, list2, list3, list4, list5);
 *
 * stream.forEach((i, v1, v2, v3, v4, v5) -> {
 *     System.out.println(i + ": " + v1 + ", " + v2 + ", " + v3 + ", " + v4 + ", " + v5");
 * });
 * }</pre>
...
public class Zipper5<T1, T2, T3, T4, T5> {
    ...
    public Zipper5(List<T1> list1, List<T2> list2, List<T3> list3, List<T4> list4, List<T5> list5) {
    ...
    /**
     * Adds a new array of elements to this {@link Zipper5}, producing a {@link Zipper6}
     * that enables parallel index-aware iteration over 6 sequences.
     *
     * <p>The number of provided elements must match the size of the existing lists to
     * maintain index alignment.</p>
     *
     * <pre>{@code
     * streamX5.zip(v1, v2, v3)
     *        .forEach((i, ...) -> { ... });
     * }</pre>
     *
     * @param values the array of new elements to include
     * @param <T6> the type of the new elements
     * @return a {@code Zipper6<T1, T2, T3, T4, T5, T6>} representing the parallel lists
     * @throws IllegalArgumentException if the number of elements does not match the existing size
     */
     @SafeVarargs    public final <T6> Zipper6 <T1, T2, T3, T4, T5, T6> zip6( T6... values) {
        ...
        return new Zipper6 <>(list1, list2, list3, list4, list5, safeList);
    }
    /**
     * Adds a new list to this {@link Zipper5}, producing a {@link Zipper6} that enables
     * parallel index-aware iteration over 6 lists.
     *
     * <p>All lists must have the same size to maintain index alignment.</p>
     *
     * <pre>{@code
     * streamX5.zipList(list6)
     *        .forEach((i, ...) -> { ... });
     * }</pre>
     *
     * @param list the new list to include in the stream
     * @param <T6> the type of elements in the new list
     * @return a {@code Zipper6<T1, T2, T3, T4, T5, T6>} representing the parallel lists
     * @throws IllegalArgumentException if the provided list size does not match existing lists
     */
    public <T6> Zipper6<T1, T2, T3, T4, T5, T6> zipList6(List<T6> list) {
        if (list.size() != size) {
            throw new IllegalArgumentException("List size mismatch");
        }
        return new Zipper6<>(list1, list2, list3, list4, list5, list);
    }


    /**
     * Performs the given action on each set of elements in parallel across 5 lists.
     * The index and corresponding elements are passed to the provided action.
     *
     * <pre>{@code
     * streamX5.forEach((i, a1, ..., a5) -> {
     *     // use i, a1, ..., a5
     * });
     * }</pre>
     *
     * @param action a lambda that receives the index and the 5 elements at that index
     */
    public void forEach5(Consumer5<Integer, T1, T2, T3, T4, T5> action) {
    ...
    /**
     * Maps each indexed group of 5 elements into a new result using the provided mapper.
     *
     * <pre>{@code
     * List<R> results = streamX5.map((i, a1, ..., a5) -> {
     *     return ...; // return type R
     * });
     * }</pre>
     *
     * @param mapper a function that receives the index and 5 elements, and returns a result
     * @param <R> the result type of the mapping function
     * @return a list of mapped results
     */
    public <R> List<R> map5(Function5<Integer, T1, T2, T3, T4, T5, R> mapper) {
     ...
     *
     * <pre>{@code
     * streamX5.filter((i, a1, ..., a5) -> condition)
     * }</pre>
     *
     * @param predicate the predicate to test each element group
     * @return a new {@code Zipper5<T1, T2, T3, T4, T5>} with filtered elements
     */
    public Zipper5<T1, T2, T3, T4, T5> filter5(Predicate5<Integer, T1, T2, T3, T4, T5> predicate) {
        ...
        return new Zipper5<>(filtered1, filtered2, filtered3, filtered4, filtered5);
    }
    /**
     * Reduces all elements into a single result using the given accumulator function.
     *
     * <pre>{@code
     * R result = streamX5.reduce(initialValue, (acc, i, a1, ..., a5) -> ...);
     * }</pre>
     *
     ...
     */
    public <R> R reduce5(R identity, Accumulator5<Integer, T1, T2, T3, T4, T5, R> accumulator) {
        R result = identity;
        for (int i = 0; i < size; i++) {
    ...
    /**
     * Converts this stream into a list of {@code Tuple5} objects,
     ...
     * </p>
     *
     * @return a list of {@code Tuple5<Integer, T1, T2, T3, T4, T5>} entries.
     */
    public List<Tuple5<T1, T2, T3, T4, T5>> asTupleList5() {
     ...
     */
    public boolean allMatch5(Predicate5<Integer, T1, T2, T3, T4, T5> predicate) {
     ...
     */
    public boolean anyMatch5(Predicate5<Integer, T1, T2, T3, T4, T5> predicate) {
        for (int i = 0; i < list1.size(); i++) {
     ...
     */
    public Zipper5<T1, T2, T3, T4, T5> peek5(Consumer5<Integer, T1, T2, T3, T4, T5> action) {
        for (int i = 0; i < list1.size(); i++) {
            action.accept(i, list1.get(i), list2.get(i), list3.get(i), list4.get(i), list5.get(i));
        }
        return this;
