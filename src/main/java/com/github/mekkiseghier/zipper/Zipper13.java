...
import com.github.mekkiseghier.zipper.accumulators.Accumulator13;
import com.github.mekkiseghier.zipper.consumers.Consumer13;
import com.github.mekkiseghier.zipper.functions.Function13;
import com.github.mekkiseghier.zipper.predicates.Predicate13;
import com.github.mekkiseghier.zipper.tuples.Tuple13;
...
/**
 * A type-safe stream-like utility for working with 13 parallel lists of values.
 * <p>
 * {@code Zipper13} enables index-aware, type-safe, and readable iteration over 13 lists in parallel.
 ...
 *
 * <pre>{@code
 * Zipper13<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> stream =
 *     Zipper.addLists(list1, list2, list3, list4, list5, list6, list7, list8, list9, list10, list11, list12, list13);
 *
 * stream.forEach((i, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13) -> {
 *     System.out.println(i + ": " + v1 + ", " + v2 + ", " + v3 + ", " + v4 + ", " + v5 + ", " + v6 + ", " + v7 + ", " + v8 + ", " + v9 + ", " + v10 + ", " + v11 + ", " + v12 + ", " + v13");
 * });
 * }</pre>
...
public class Zipper13<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> {
    ...
    public Zipper13(List<T1> list1, List<T2> list2, List<T3> list3, List<T4> list4, List<T5> list5, List<T6> list6, List<T7> list7, List<T8> list8, List<T9> list9, List<T10> list10, List<T11> list11, List<T12> list12, List<T13> list13) {
    ...
    /**
     * Adds a new array of elements to this {@link Zipper13}, producing a {@link Zipper14}
     * that enables parallel index-aware iteration over 14 sequences.
     *
     * <p>The number of provided elements must match the size of the existing lists to
     * maintain index alignment.</p>
     *
     * <pre>{@code
     * streamX13.addElements(v1, v2, v3)
     *        .forEach((i, ...) -> { ... });
     * }</pre>
     *
     * @param values the array of new elements to include
     * @param <T14> the type of the new elements
     * @return a {@code Zipper14<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14>} representing the parallel lists
     * @throws IllegalArgumentException if the number of elements does not match the existing size
     */
     @SafeVarargs    public final <T14> Zipper14 <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14> addElements( T14... values) {
        ...
        return new Zipper14 <>(list1, list2, list3, list4, list5, list6, list7, list8, list9, list10, list11, list12, list13, safeList);
    }


    /**
     * Adds a new list to this {@link Zipper13}, producing a {@link Zipper14} that enables
     * parallel index-aware iteration over 14 lists.
     *
     * <p>All lists must have the same size to maintain index alignment.</p>
     *
     * <pre>{@code
     * streamX13.addList(list14)
     *        .forEach((i, ...) -> { ... });
     * }</pre>
     *
     * @param list the new list to include in the stream
     * @param <T14> the type of elements in the new list
     * @return a {@code Zipper14<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14>} representing the parallel lists
     * @throws IllegalArgumentException if the provided list size does not match existing lists
     */
    public <T14> Zipper14<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14> addList(List<T14> list) {
        if (list.size() != size) {
            throw new IllegalArgumentException("List size mismatch");
        }
        return new Zipper14<>(list1, list2, list3, list4, list5, list6, list7, list8, list9, list10, list11, list12, list13, list);
    }



    /**
     * Performs the given action on each set of elements in parallel across 13 lists.
     * The index and corresponding elements are passed to the provided action.
     *
     * <pre>{@code
     * streamX13.forEach((i, a1, ..., a13) -> {
     *     // use i, a1, ..., a13
     * });
     * }</pre>
     *
     * @param action a lambda that receives the index and the 13 elements at that index
     */
    ...
    /**
     * Maps each indexed group of 13 elements into a new result using the provided mapper.
     *
     * <pre>{@code
     * List<R> results = streamX13.map((i, a1, ..., a13) -> {
     *     return ...; // return type R
     * });
     * }</pre>
     *
     * @param mapper a function that receives the index and 13 elements, and returns a result
     ...
     *
     * <pre>{@code
     * streamX13.filter((i, a1, ..., a13) -> condition)
     * }</pre>
     *
     * @param predicate the predicate to test each element group
     * @return a new {@code Zipper13<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13>} with filtered elements
     */
    public Zipper13<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> filter(Predicate13<Integer, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> predicate) {
        ...
        return new Zipper13<>(filtered1, filtered2, filtered3, filtered4, filtered5, filtered6, filtered7, filtered8, filtered9, filtered10, filtered11, filtered12, filtered13);
    }
    /**
     * Reduces all elements into a single result using the given accumulator function.
     *
     * <pre>{@code
     * R result = streamX13.reduce(initialValue, (acc, i, a1, ..., a13) -> ...);
     * }</pre>
     *
    ...
    /**
     * Converts this stream into a list of {@code Tuple13} objects,
     ...
     * </p>
     *
     * @return a list of {@code Tuple13<Integer, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13>} entries.
     */
     ...
     */
    public Zipper13<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> peek(Consumer13<Integer, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> action) {
        for (int i = 0; i < list1.size(); i++) {
            action.accept(i, list1.get(i), list2.get(i), list3.get(i), list4.get(i), list5.get(i), list6.get(i), list7.get(i), list8.get(i), list9.get(i), list10.get(i), list11.get(i), list12.get(i), list13.get(i));
        }
        return this;
